...
function createJQComponent(jqueryObj) {
  if (!jqueryObj || (typeof jqueryObj !== 'function' && typeof jqueryObj !== 'object')) {
    return
  }
  class JQFactory extends React.Component {
    ...
    constructor(props) {
      ...
      this.instance = typeof jqueryObj === 'function' ? new jqueryObj() : jqueryObj
    }
    componentDidMount() {
  ...
  class VueFactory extends React.Component {
    ...
    componentDidMount() {
      ...
      Object.keys(vueObj.props).forEach(key => {
        if (typeof vueObj.props[key] != 'function' && typeof vueObj.props[key] != 'object') {
          flag = flag && !amisData[key]
        }
      })
      const { data, props, ...rest } = (vueObj = typeof vueObj === 'function' ? new vueObj() : vueObj)
      // 传入的Vue属性
      this.vm = new Vue({
        data: extendObject(amisData, typeof data === 'function' ? data() : data),
        ...rest,
        props: flag ? {} : props ?? rest.props,
        _props: props ?? rest.props
      })
    ...
    componentDidUpdate() {
      Object.keys(this.props.$schema).forEach(key => {
        ...
        else if (typeof this.props[key] == 'object' && this.props[key]) {
          if (Object.prototype.toString.call(this.vm[key]) === '[object Array]') {
            this.vm[key] = this.props[key]
          } else if (this.vm[key]) {
            this.vm[key] = Object.assign(this.vm[key], this.props[key])
          } else {
            this.vm[key] = this.props[key]
          }
        } else if (typeof this.props[key] == 'boolean') {
          this.vm[key] = this.props[key] ? true : false
        } else if (typeof this.props[key] == 'number') {
          this.vm[key] = this.props[key] ? this.props[key] : 0
        } else if (typeof this.props[key] == 'string') {
          this.vm[key] = this.props[key] ? this.props[key] : ''
        }
      })
      this.vm.$forceUpdate()
    }
